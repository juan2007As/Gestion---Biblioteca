...
class Libro:
    def __init__(self, id_libro, titulo, autor, editorial, año, genero, disponible=True, id=None):
        self.id = id
    ...
    def guardar(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO Libro (id_libro, titulo, autor, editorial, año, genero, disponible)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (self.id_libro, self.titulo, self.autor, self.editorial, self.año, self.genero, self.disponible))
        conn.commit()
        conn.close()
    ...
    def obtener_todos():
        ...
        conn.close()
        return [Libro(id=f[0], id_libro=f[1], titulo=f[2], autor=f[3],editorial=f[4], año=f[5], genero=f[6], disponible=bool(f[7])) for f in filas]
    ...
    def buscar_por_termino(termino):
        ...
        filas = cursor.fetchall()
        conn.close()
        return [Libro(id=f[0], id_libro=f[1], titulo=f[2], autor=f[3],
                     editorial=f[4], año=f[5], genero=f[6], disponible=bool(f[7])) for f in filas]
    ...
    def obtener_por_id(id_libro):
        ...
        conn.close()
        if fila:
            return Libro(id=fila[0], id_libro=fila[1], titulo=fila[2], autor=fila[3],
                        editorial=fila[4], año=fila[5], genero=fila[6], disponible=bool(fila[7]))
        return None

class Usuario:
    def __init__(self, nombre, apellido, documento, telefono, email, rol="normal", id=None):
        self.id = id
        self.nombre = nombre
    ...
    def guardar(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO Usuario (nombre, apellido, documento, telefono, email, rol)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (self.nombre, self.apellido, self.documento, self.telefono, self.email, self.rol))
        conn.commit()
        conn.close()
    ...
    def obtener_todos():
        ...
        filas = cursor.fetchall()
        conn.close()
        return [Usuario(id=f[0], nombre=f[1], apellido=f[2], documento=f[3],
                        telefono=f[4], email=f[5], rol=f[6] if len(f) > 6 else "normal") for f in filas]
    ...
    def obtener_por_documento(documento):
        ...
        conn.close()
        if fila:
            return Usuario(id=fila[0], nombre=fila[1], apellido=fila[2], documento=fila[3],
                          telefono=fila[4], email=fila[5], rol=fila[6] if len(fila) > 6 else "normal")
        return None
...
def agregar_libro():
    ...
    libro = Libro(id_libro=nuevo_id, titulo=titulo, autor=autor, editorial=editorial, año=año, genero=genero, disponible=True)
    libro.guardar()
...
def buscar_libro(criterio,valor,solo_disponibles=True):
    ...
    resultados = [Libro(id=f[0], id_libro=f[1], titulo=f[2], autor=f[3], editorial=f[4], año=f[5], genero=f[6], disponible=bool(f[7])) for f in filas]
...
def Reportes():
    while True:
        print("\n--- Reportes ---")
        print("1. Total de préstamos / activos")
        print("2. Usuarios con multas")
        print("3. Estadísticas por género")
        print("4. Exportar Libros a CSV")
        print("5. Volver")
        op = input("Seleccione: ").strip()

        if op == "1":
            reporte_total_prestados()
        elif op == "2":
            reporte_usuarios_con_multas()
        elif op == "3":
            reporte_estadisticas_por_genero()
        elif op == "4":
            exportar_csv()
